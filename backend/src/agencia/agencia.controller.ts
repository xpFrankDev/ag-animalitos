...
export class AgenciaController {
  ...
  @Get('tickets') listarTickets(@Request() solicitud: { usuario: { sub: string; tipo_usuario: string } }, @Query('estado') estado?: string) { return this.servicio.listarTickets(solicitud.usuario as never, estado); }
