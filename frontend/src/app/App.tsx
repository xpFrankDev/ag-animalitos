...
export function App() {
  ...
  const salir = () => { localStorage.removeItem('ag_sesion'); establecerSesion(null); establecerMenuMovil(false); establecerConsulta(null); };
  ...
  return <main>
    ...
    {sesion ? <><AgenciaVentasRuta token={sesion.token} alVencerSesion={salir} />{consultaAbierta && <ConsultaAgenciaRuta token={sesion.token} vista={consultaAbierta} alCerrar={() => establecerConsulta(null)} />}</> : <InicioSesionRuta alIngresar={(nuevaSesion) => { localStorage.setItem('ag_sesion', JSON.stringify(nuevaSesion)); establecerSesion(nuevaSesion); }} />}
  </main>;
}
