...
export function AgenciaVentasRuta({ token, usuario }: { token: string; usuario: string }) {
  ...
  return <section className="ventas">
    <div className="encabezado-ventas"><div><p className="etiqueta">AGENCIA · {inicio.agencia.codigo_agencia}</p><h1>{inicio.agencia.nombre_agencia}</h1><p>{usuario} · {t('cupo')}: <strong>{inicio.agencia.cupo_animal.toFixed(2)}</strong> · Comisión: {inicio.agencia.comision_porcentaje}%</p></div><time className="hora-venezuela">🇻🇪 {horaVenezuela}</time></div>
    <form className="rejilla-ventas" onSubmit={emitir}>
      ...
      <section className="tarjeta panel-operacion"><div className="total total-superior"><span>Total ticket</span><strong>${total.toFixed(2)}</strong></div><label>{t('monto')}<div className="campo-monto"><span>$</span><input inputMode="decimal" value={monto} onKeyDown={manejarTecla} onChange={(evento) => establecerMonto(evento.target.value)} /></div></label><button type="button" className="boton-secundario ancho-completo" onClick={agregarJugada}>{t('agregar')} ↵</button><div className="titulo-panel"><h2>2. {t('sorteos')}</h2><span>Cierre: {inicio.agencia.minutos_cierre} min</span></div><div className="lista-sorteos">{inicio.horarios.map((horario) => <label className={`opcion-sorteo ${horariosSeleccionados.includes(horario.pk_horario_sorteo) ? 'seleccionado' : ''} ${!horario.disponible ? 'cerrado' : ''}`} key={horario.pk_horario_sorteo}><input type="checkbox" disabled={!horario.disponible} checked={horariosSeleccionados.includes(horario.pk_horario_sorteo)} onChange={() => alternar(horario.pk_horario_sorteo, horariosSeleccionados, establecerHorarios)} /><span><b>{horario.sorteo}</b><small>{horario.hora}{!horario.disponible ? ' · cerrado' : ''}</small></span></label>)}</div></section>
      <aside className="tarjeta panel-ticket"><h2>3. {t('jugadas')}</h2><div className="lista-jugadas">{jugadas.length === 0 ? <p className="vacio">{t('no_hay_jugadas')}</p> : jugadas.map((jugada, indice) => <div className="fila-jugada" key={`${jugada.fk_animal}-${jugada.fk_horario_sorteo}`}><span>{animalesPorId.get(jugada.fk_animal)?.icono} <b>{animalesPorId.get(jugada.fk_animal)?.codigo_animal}</b> · {horariosPorId.get(jugada.fk_horario_sorteo)?.sorteo} {horariosPorId.get(jugada.fk_horario_sorteo)?.hora}</span><strong>${jugada.monto.toFixed(2)}</strong><button type="button" aria-label="Eliminar jugada" onClick={() => establecerJugadas(jugadas.filter((_, posicion) => posicion !== indice))}>×</button></div>)}</div><button className="boton-primario ancho-completo" disabled={!jugadas.length || emitiendo}>{emitiendo ? '…' : t('emitir')}</button></aside>
    </form>
    <section className="acciones-ticket tarjeta"><span>Acciones de ticket</span><button type="button">Repetir ticket</button><button type="button">Pagar ticket</button><button type="button">Eliminar ticket</button><button type="button">Visualizar ticket</button></section>
