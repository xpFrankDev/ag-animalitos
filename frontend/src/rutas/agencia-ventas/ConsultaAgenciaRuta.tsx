...
export function ConsultaAgenciaRuta({ token, vista, alCerrar }: { token: string; vista: VistaConsulta; alCerrar: () => void }) {
  ...
  const jugadasTicketOrdenadas = useMemo(() => [...(ticketActivo?.jugadas ?? [])].sort((primera, segunda) => (primera.horario_sorteo?.hora ?? '').localeCompare(segunda.horario_sorteo?.hora ?? '')), [ticketActivo]);
  ...
  return <div className="fondo-consulta" role="presentation"><section className="ventana-consulta tarjeta" role="dialog" aria-modal="true" aria-labelledby="titulo-consulta">
    ...
    {cargando ? <p className="estado-consulta">{t('cargando')}</p> : error ? <p className="estado-consulta mensaje-error">{error}</p> : <div className="contenido-consulta">
      ...
      {vista === 'tickets' && (tickets.length ? <div className="detalle-tickets"><div className="tabla-consulta lista-tickets">{tickets.map((ticket) => <button type="button" className={`fila-consulta ticket ${ticket.serial === ticketSeleccionado ? 'seleccionado' : ''}`} key={ticket.serial} onClick={() => establecerTicketSeleccionado(ticket.serial)}><span><strong>Ticket #{ticket.numero_ticket}</strong><small>{ticket.fecha_juego} · Serial {ticket.serial}</small></span><span className={`estado-ticket ${ticket.estado.toLowerCase()}`}>{etiquetaEstado(ticket.estado)}</span><strong>{formatearMonto(ticket.total_jugado)}</strong></button>)}</div>{ticketActivo && <aside className="detalle-ticket"><header><span>Ticket #{ticketActivo.numero_ticket}</span><span className={`estado-ticket ${ticketActivo.estado.toLowerCase()}`}>{etiquetaEstado(ticketActivo.estado)}</span></header><span>{ticketActivo.fecha_juego} · Serial {ticketActivo.serial}</span><strong>{t('total')}: {formatearMonto(ticketActivo.total_jugado)}</strong><span>{t('estado_pagado')}: {formatearMonto(ticketActivo.monto_pagado)}</span><div className="jugadas-ticket-detalle">{jugadasTicketOrdenadas.map((jugada, indice) => <p key={indice}><span><b>{jugada.horario_sorteo?.hora}</b> · {jugada.horario_sorteo?.sorteo?.nombre}</span><span>{jugada.animal?.codigo_animal} · {jugada.animal?.nombre}</span><strong>{formatearMonto(jugada.monto)}</strong></p>)}</div></aside>}</div> : <p className="estado-consulta">{t('sin_tickets')}</p>)}
      {vista === 'resumen' && resumen && <><div className="metricas-resumen"><div><span>{t('total_vendido')}</span><strong><NumeroAnimado valor={resumen.total_vendido} formato={formatearMonto} /></strong></div><div><span>{t('total_premiado')}</span><strong><NumeroAnimado valor={resumen.total_premiado} formato={formatearMonto} /></strong></div><div><span>Comisión {resumen.porcentaje_comision.toFixed(0)}%</span><strong><NumeroAnimado valor={resumen.total_comision} formato={formatearMonto} /></strong></div><div><span>{t('resto')}</span><strong><NumeroAnimado valor={resumen.resto} formato={formatearMonto} /></strong></div></div>{resumen.tickets.length ? <div className="tabla-consulta">{resumen.tickets.map((ticket) => <div className="fila-consulta ticket" key={ticket.serial}><span><strong>#{ticket.numero_ticket}</strong><small>{ticket.fecha_juego} · {ticket.serial}</small></span><span className={`estado-ticket ${ticket.estado.toLowerCase()}`}>{etiquetaEstado(ticket.estado)}</span><strong>{formatearMonto(ticket.total_jugado)}</strong></div>)}</div> : <p className="estado-consulta">{t('sin_datos_rango')}</p>}</>}
    </div>}
