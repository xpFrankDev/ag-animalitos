...
type Inicio = {
  ...
  agencias: { pk_agencia: string; codigo_agencia: string; nombre_agencia: string; activa: boolean; operador: string; equipo_asignado: boolean }[];
...
export function OperacionRuta({ token, alVencerSesion }: { token: string; alVencerSesion: () => void }) {
  ...
  async function liberarEquipo(pkAgencia: string) {
    try { const respuesta = await llamarApi<{ mensaje: string }>(`/operacion/agencias/${pkAgencia}/liberar-serial`, { method: 'POST' }, token); establecerMensaje(respuesta.mensaje); await cargar(); }
    catch (error) { establecerMensaje(error instanceof Error ? error.message : t('consulta_error')); }
  }
  ...
  return <section className="operacion">
    ...
    {vista === 'agencias' && <div className="tablero-operacion">{inicio.perfil.tipo_usuario === 'BANQUERO' && <section className="tarjeta bloque-operacion"><h2>{t('gruperos')}</h2>{inicio.gruperos.length ? <div className="lista-operacion">{inicio.gruperos.map((grupero) => <div key={grupero.pk_grupero}><strong>{grupero.nombre_completo}</strong><span className={grupero.activo ? 'estado-activo' : 'estado-inactivo'}>{grupero.activo ? t('activo') : t('inactivo')}</span></div>)}</div> : <p className="vacio-operacion">{t('sin_gruperos')}</p>}</section>}<section className="tarjeta bloque-operacion"><h2>{t('agencias')}</h2>{inicio.agencias.length ? <div className="tabla-operacion">{inicio.agencias.map((agencia) => <div key={agencia.pk_agencia}><span><strong>{agencia.nombre_agencia}</strong><small>{agencia.codigo_agencia} · {agencia.operador} · {agencia.equipo_asignado ? t('equipo_asignado') : t('sin_equipo')}</small></span><span className={agencia.activa ? 'estado-activo' : 'estado-inactivo'}>{agencia.activa ? t('activo') : t('inactivo')}</span>{agencia.equipo_asignado && <button type="button" className="boton-secundario" onClick={() => void liberarEquipo(agencia.pk_agencia)}>{t('liberar_equipo')}</button>}</div>)}</div> : <p className="vacio-operacion">{t('sin_agencias')}</p>}</section></div>}
